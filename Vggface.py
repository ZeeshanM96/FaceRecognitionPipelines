...
def extract_face_embedding_vgg(image_path):
    # Load the image
    ...
    if img is None:
        ...
        return None
    try:
        # Perform face embedding using VGG-Face model
...
def verify_face(image_path1, image_path2):
    ...
    try:
        verification = DeepFace.verify(image_path1, image_path2, model_name='VGG-Face', enforce_detection=False)
        return verification["verified"]
    except Exception as e:
        print(f"An error occurred during face verification: {e}")
        return None

def extract_embeddings_vgg(folder_path, reference_image_path, app):
    embeddings = {}
    verifications = {}

    # Extract embedding for the preprocessed (aligned) reference image
    reference_embedding = DeepFace.represent(reference_image_path, model_name='VGG-Face', enforce_detection=False)
    ...
    for image_name in os.listdir(folder_path):
        ...
        if aligned_face is not None:
            embedding = DeepFace.represent(aligned_face, model_name='VGG-Face', enforce_detection=False)
