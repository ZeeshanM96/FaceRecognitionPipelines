...
from alignface import align_face_1

def extract_face_embedding_arcface(image_path):
    # Load the image
    ...
    try:
        ...
        embedding = DeepFace.represent(img, model_name='ArcFace', detector_backend='retinaface',enforce_detection=False)
        return embedding
...
def verify_face(image_path1, image_path2):
    ...
    try:
        verification = DeepFace.verify(image_path1, image_path2, model_name='ArcFace', detector_backend='retinaface', enforce_detection=False)
        return verification["verified"]
    except Exception as e:
        print(f"An error occurred during face verification: {e}")
        return None

def extract_embeddings_arcface(folder_path, reference_image_path, app):
    embeddings = {}
    verifications = {}

    # Extract embedding for the preprocessed (aligned) reference image
    reference_embedding = DeepFace.represent(reference_image_path, model_name='ArcFace', enforce_detection=False)
